...
public class IconRoundCornerProgressBar extends BaseRoundCornerProgressBar {
    ...
    @Override
    public void setBackgroundLayoutSize(LinearLayout layoutBackground, int width, int height) {
        /*if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
    ...
    @Override
    protected float setLayoutProgressHeight(float ratio) {
        return (ratio > 0) ? (getBackgroundHeight() - (headerWidth + (getPadding() * 2))) / ratio : 0;
    }
